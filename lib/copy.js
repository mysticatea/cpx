...
module.exports = function copy(source, outputDir, options, callback) {
    ...
    const promise = co(function* () {
        console.assert(typeof source === "string", "'source' should be a string.")
        console.assert(source.trim().length >= 1, "'source' should not be empty.")
        console.assert(typeof outputDir === "string", "'outputDir' should be a string.")
        console.assert(outputDir.trim().length >= 1, "'outputDir' should not be empty.")
        if (options != null) {
        ...
        if (options.clean) {
            ...
            if (output !== options.source) {
                yield applyAction(output, options, (targetPath) => removeFile(targetPath))
            }
        }
