...
module.exports = class Watcher extends EventEmitter {
    ...
    open() {
        ...
        this.addDirectory(this.baseDir).then(() => {
            this.onReady()
        }).catch(error => {
            this.onError(error)
        })
    ...
    addDirectory(root) {
        ...
        return walkDirectories(root, this.dereference, co.wrap(function* (dirPath, files) {
            if (this.trigger == null || this.watchers.has(dirPath)) {
                return
            }

            // Skip the content of symbolic links by default.
            if (!this.dereference && (yield fs.lstat(dirPath)).isSymbolicLink()) {
            ...
            const watcher = fs.watch(dirPath)
                .on("change", (type, filename) => {
                    ...
                    const sourcePath = normalizePath(path.join(dirPath, filename))
            ...
            for (const entry of queue.entries()) {
                ...
                if (type === "remove") {
                    try {
                        ...
                        if (error.code === "EPERM" && this.shouldRetry(sourcePath)) {
                    ...
                    try {
                        ...
                        if ((error.code === "ENOENT" || error.code === "EPERM") && this.shouldRetry(sourcePath)) {
                            debug("Watcher#onTrigger~retry", type, sourcePath)
                            if (type === "add") {
                                this.onAdded(sourcePath)
                            }
