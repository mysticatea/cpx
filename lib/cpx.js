...
const createWatcher = require("chokidar").watch
...
function normalizePath(originalPath) {
    ...
    let normalizedPath = path.relative(
        process.cwd(),
...
function doAllSimply(cpx, pattern, action) {
    new glob.Glob(pattern, {nodir: !cpx.includeEmptyDirs, silent: true})
...
function doAll(cpx, pattern, action, cb) {
    ...
    .on("match", (sourcePath) => {
        ...
        count += 1
        action.call(cpx, sourcePath, (err) => {
            count -= 1
...
module.exports = class Cpx extends EventEmitter {
    ...
    get base() {
        if (this[BASE_DIR] == null) {
            this[BASE_DIR] =
                normalizePath(getBasePath(new glob.Glob(this.source)))
        }
     ...
     *
     * @param {string} sourcePath - A path to convert.
    ...
    src2dst(sourcePath) {
        ...
        if (this.base === ".") {
            return path.join(this.outDir, sourcePath)
        }
        return sourcePath.replace(this.base, this.outDir)
    }
    ...
    enqueueCopy(srcPath, cb) {
        ...
        this[QUEUE].push(next => {
            mkdir(path.dirname(dstPath), next)
        })
        this[QUEUE].push(next => {
            copyFile(
                srcPath,
     ...
     * Remove a file.
     *
     * @param {string} targetPath - A file path to remove.
     * @param {function} [cb = null] - A callback function.
     * @returns {void}
     */
    enqueueRemove(targetPath, cb) {
        ...
        this[QUEUE].push(next => {
            if (stat && stat.isDirectory()) {
                fs.rmdir(targetPath, (err) => {
                    if (err == null) {
                        this.emit("remove", {path: targetPath})
                    }
        ...
        this[QUEUE].push(next => {
            fs.rmdir(path.dirname(targetPath), () => {
                next()
                if (cb != null) {
                    cb(lastError)
                }
    ...
    cleanSync() {
        ...
        for (const targetPath of glob.sync(
            dest,
            {
